...
# liefert die Autobahn-Nummer einer Anschluss-Autobahn -------------------------
sub get_bab_nach
{
	# Attribute: Name einer Übergangsstelle($nach_alt)
	#            Nummer der "alten" Autobahn($bab_alt)
	# return: Nummer der zugehörigen neuen Autobahn(bab_neu)
	my $self = shift;
  my $nach_alt = $_[0];
  my $bab_alt  = $_[1];
  my $datei    = $self->{'-datei'};
  ...
  for (my $i=0; $i<=$#zeilen; $i++)
  {
    ...
    if ($von eq $nach_alt && $bab ne $bab_alt)
    {
      $bab_neu = $bab;
    }
  }
...
# liefert alle Autobahnübergänge auf einer Autobahn ----------------------------
sub get_k_typ2
{
	# Argumente: Nummer einer Bundesautobahn ($bab_alt)
	# return: ein Array aller Autobahnübergänge(NACH) dieser Autobahn (@uebergaenge)
	my $self = shift;
  my $bab_alt = $_[0];
  my $datei   = $self->{'-datei'};
  my @uebergaenge;
  ...
  for (my $i=0; $i<=$#zeilen; $i++)
  {
    ...
    if ($bab eq $bab_alt && ($k_typ2 eq "AD  " || $k_typ2 eq "AK  "))
    {
...
# prüft, ob der Zielpunkt auf der angegebenen Bundesautobahn liegt
sub is_nach
{
  # Argumente: Nummer einer Bundesautobahn(bab_alt)
  # return: den gefundenen Zielpunkt oder 0(wenn nicht gefunden)
	my $self = shift;
  ...
  for (my $i=0; $i<=$#zeilen; $i++)
  {
    ...
    if ($nach eq $nach_auswahl && $bab eq $bab_alt)
    {
    	$treffer = $nach;
    }
  }
...
# hole Nummer der Bundesautobahn auf der sich der Startpunkt befindet ----------
sub get_bab_von
{
  # Argumente: keine
  # return: Nummer der Bundesautobahn (bab)
  my $self = shift;
  my $von_auswahl = $self->{'-von_auswahl'};
  my $datei       = $self->{'-datei'};
  my $bab_neu;
