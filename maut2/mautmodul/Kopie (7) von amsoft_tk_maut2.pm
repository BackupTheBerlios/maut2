...
#        }
	      ...
	      {
 					$self->set_pfad($such_von, $uebergaenge[$k], $bab_such_von);
					# die REKURSION
  	...
  	for (my $i=0; $i<@daten; $i++)
		{
			...
			my $k_nr2  = $daten[$i]->{'-k_nr2'};
			my $km     = $daten[$i]->{'-km'};
			if ($bab eq $pfad_bab && $k_nr1 ge $pfad_von_nr1 && $k_nr2 le $pfad_nach_nr2)
			{
      	$pfad_km = $pfad_km + $km;
			}
  	}
  	...
  	for (my $i=0; $i<@daten; $i++)
		{
			...
			my $k_nr2  = $daten[$i]->{'-k_nr2'};
			my $km     = $daten[$i]->{'-km'};
			if ($bab eq $pfad_bab && $k_nr2 le $pfad_von_nr1 && $k_nr1 ge $pfad_nach_nr2)
			{
      	$pfad_km = $pfad_km + $km;
			}
  	}
