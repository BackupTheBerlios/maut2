  ...
  if ($pfad_von_nr1_format < $pfad_nach_nr2_format)
  {
    for (my $i=0; $i<@daten; $i++)
    {
      ...
      if ($bab eq $pfad_bab)
      {
        ...
        if ($k_nr1 eq "-   ")
        {
          # wenn Knotennummer1 "-   " ist und es der Anfang einer BAB ist
          if ($k_nr1 eq "-   " && $daten[$i-1]->{'-bab'} ne $bab)
          {
            $k_nr1_format = "0   ";
            print "-anfang bab auf:$k_nr1_format\n";
          }

          # wenn Knotennummer1 "-   " ist und nicht am Anfang der BAB ist
          if ($k_nr1 eq "-   " && $daten[$i-1]->{'-bab'} eq $bab)
          {
