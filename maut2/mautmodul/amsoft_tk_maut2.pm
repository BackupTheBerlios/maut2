...
sub new
{
  ...
  {
    $pfade[$i] = {};
    $pfade[$i]->{'-pfad_vorher'} = 0;
    $pfade[$i]->{'-pfad_lfd'}    = 0;
    $pfade[$i]->{'-pfad_bab'}    = "  0";
    $pfade[$i]->{'-pfad_von'}    = "AAA";
    $pfade[$i]->{'-pfad_nach'}   = "BBB";
    $pfade[$i]->{'-pfad_km'}     = 0.0;
    $pfade[$i]->{'-km_ges'}      = 0.0;
  }
        # eine Referenz aus diesem Array erzeugen und übergeben
...
sub fra_unten
{
  ...
  $lbl_unten_2 = $fra_unten_2->Label(-text   => "",
		    					 	 #-width  => 38,
		    					 	 -justify => 'left',
		    					 	 -anchor => 'w',
		    					 	 #-font   => $font2,
...
sub get_strecke
{
  ...
  #
	...
	my $pfad_vorher  = $_[3];  # Pfad-Nummer des vorhergegangenen Pfades (dieser Suche)
	my $pfad_lfd_neu;          # für interne Weitergabe
  ...
  my @pfade      = @$pfade;       # !!! die Referenz dereferenzieren !!!
  ...
  my $lbl_unten_2 = $self->{'-lbl_unten_2'};
  			...
  			# REKURSION nur X-mal durchlaufen
  if ($rek_zaehler < 4)
  {
		print "---------------------------------------------------\n";
		print "------ in REKURSION $rek_zaehler: von: $such_von\n";
      ...
      my $bab_puffer;
	    for (my $i=0; $i<$#daten; $i++)
	    {
	      ...
	      if ($von eq $such_von)
	      {
          	# damit es die "vorherige BAB nicht wieder anzeigt
	      	foreach my $str (@babnummern)
	      	{
						if ((index $str, $bab) != -1)
						{
            	$bab_puffer = $bab;
							#print "in foreatsch:---$bab_puffer---\n";
						}
					}
					if (defined $bab_puffer)
					{
          	$bab_such_von = $bab_puffer;
          	#print "in defined puffer:---$bab_such_von---\n";
					}
					else
					{
          	$bab_such_von = 0;
          	#print "in else         :---$bab_such_von---\n";
					}
          #print "nach foreatsch bab:-$bab_such_von---\n";
	      }
      ...
      print "--- Suche ausgehend von   : $such_von\n";
	    ...
	    for (my $i=0; $i<$#daten; $i++)
	    {
          ...
          print "--------------------------------------\n";
	        ...
	        print "--------------------------------------\n";
	        my $string = $self->zeige_verbindung($pfad_vorher);
	        $lbl_unten_2->configure(-text =>
	        	"Treffer auf BAB: $bab \nVerbindung über:  $string
	        	...");
	        $knz_treffer = 1;
        ...
        print "--------------------------------- los gehts:\n";
	      	...
	      	# und: get_strecke !REKURSIV! aufrufen (die Verzweigung geht los, grrr)
	      my $anzahl = @uebergaenge;
	      for (my $k=0; $k<$anzahl; $k++)
	      {
          if (!defined $pfad_vorher)
          {
          	$pfad_vorher = 0;
          }
          my $pfad_lfd;
 					$pfad_lfd = $self->set_pfad($such_von, $uebergaenge[$k], $bab_such_von, $pfad_vorher);
          print "pfad_lfd  :$pfad_lfd\n";
          print "uebergang : $uebergaenge[$k]\n";
          print "pfad_km   : $$pfade[$pfad_lfd]->{'-pfad_km'} \n";
          print "---\n";
					# die REKURSION
          $self->get_strecke($uebergaenge[$k], $such_nach, $rek_zaehler+1, $pfad_lfd);
	      }
	      print "---------------------------------\n";
	    }
	  }
  }
  #print "-am ENDE:@babnummern\n";
...
#
sub zeige_verbindung
{
  # Attribute: Pfad-Nummer des vorherigen Pfades ($pfad_vorher)
  # return:    String, der die Ausgabe der Übergänge enthält
	my $self = shift;
	my $pfad_vorher = $_[0];
	my $rueckgabestring;

  my @pfad_treffer;
  #@pfad_treffer = $self->hole_pfade($pfad_vorher);
  @pfad_treffer = $self->hole_pfade($pfad_vorher);
...
sub hole_pfade
{
  # Argumente: Pfad-Nummer ($pfad_treffer) (zuerst Nr. des Treffers, dann rückwärts)
  # return   : Array der Pfadnummern des gesamten Trefferpfades
	my $self = shift;
	my $pfad_treffer = $_[0];
	my $pfade = $self->{'-ref_pfade'}; # Die Pfade als Referenz auf ein Hash-Array
	my @pfade = @$pfade;       # die Referenz dereferenziert
  my @pfad_alles = ();
	...
	$uebergang1 = $$pfade[$pfad_treffer]->{'-pfad_nach'};
	if ($$pfade[$pfad_treffer]->{'-pfad_vorher'} != 0)
	{
  	my $vorher1 = $$pfade[$pfad_treffer]->{'-pfad_vorher'};
  	$uebergang2 = $$pfade[$vorher1]->{'-pfad_nach'};
  	if ($$pfade[$vorher1]->{'-pfad_vorher'} != 0)
  	{
    	my $vorher2 = $$pfade[$vorher1]->{'-pfad_vorher'};
    	$uebergang3 = $$pfade[$vorher2]->{'-pfad_nach'};
  		if ($$pfade[$vorher2]->{'-pfad_vorher'} != 0)
  		{
    		my $vorher3 = $$pfade[$vorher2]->{'-pfad_vorher'};
    		$uebergang4 = $$pfade[$vorher3]->{'-pfad_nach'};
  		}
  	}
	}

  push(@pfad_alles, $uebergang1);
  push(@pfad_alles, $uebergang2);
  push(@pfad_alles, $uebergang3);
  push(@pfad_alles, $uebergang4);
	return @pfad_alles;
}
...
sub set_pfad
{
  ...
  # return: (laufende)Pfadnummer dieses Pfadabschnittes ($pfad_lfd)
	my $self = shift;
  ...
  my $pfad_bab     = $_[2];
  my $pfad_vorher  = $_[3];
  my $pfad_lfd;              # laufende Nummer des Streckenabschnittes
  my $pfad_km  = 0.0;        # zusammenaddierte Kilometer eines Pfadabschnittes
  my $km_ges   = 0.0;        # zusammenaddierte Kilometer des Gesamt-Pfades
  ...
  $pfad_von_nr1_format  = $self->format_knoten($pfad_von_nr1);
	$pfad_nach_nr2_format = $self->format_knoten($pfad_nach_nr2);
  			...
  			# die Vorwärts-Suche
  #print "-nr1_format:$pfad_von_nr1_format\n";
	#print "-nr2_format:$pfad_nach_nr2_format\n";
    ...
    for (my $i=0; $i<@daten; $i++)
    {
      ...
      if ($bab eq $pfad_bab)
      {
        ...
        if ($k_nr1 eq "-   ")
        {
          ...
          # wenn Knotennummer1 "-   " ist und nicht am Anfang der BAB ist
          if ($k_nr1_format eq "-   " && $daten[$i-1]->{'-bab'} eq $bab)
          {
          	$k_nr1_format = $self->format_knoten($daten[$i-1]->{'-k_nr1'});
          	#print "format im 1.:$k_nr1_format\n";
            if ($k_nr1_format eq "-   " && $daten[$i-2]->{'-bab'} eq $bab)
            {
            	$k_nr1_format = $self->format_knoten($daten[$i-2]->{'-k_nr1'});
            	#print "format im 2.:$k_nr1_format\n";
              if ($k_nr1_format eq "-   " && $daten[$i-3]->{'-bab'} eq $bab)
              {
              	$k_nr1_format = $self->format_knoten($daten[$i-3]->{'-k_nr1'});
              	#print "format im 3.:$k_nr1_format\n";
                if ($k_nr1_format eq "-   " && $daten[$i-4]->{'-bab'} eq $bab)
              	{
              		$k_nr1_format = $self->format_knoten($daten[$i-4]->{'-k_nr1'});
              		#print "format im 4.:$k_nr1_format\n";
                  if ($k_nr1_format eq "-   " && $daten[$i-5]->{'-bab'} eq $bab)
              		{
              			$k_nr1_format = $self->format_knoten($daten[$i-5]->{'-k_nr1'});
              			#print "format im 5.:$k_nr1_format\n";
                    if ($k_nr1_format eq "-   " && $daten[$i-6]->{'-bab'} eq $bab)
              			{
              				$k_nr1_format = $self->format_knoten($daten[$i-6]->{'-k_nr1'});
              				#print "format im 5.:$k_nr1_format\n";
                      if ($k_nr1_format eq "-   " && $daten[$i-7]->{'-bab'} eq $bab)
              				{
              					$k_nr1_format = $self->format_knoten($daten[$i-7]->{'-k_nr1'});
              					#print "format im 5.:$k_nr1_format\n";
                        if ($k_nr1_format eq "-   " && $daten[$i-8]->{'-bab'} eq $bab)
              					{
              						$k_nr1_format = $self->format_knoten($daten[$i-8]->{'-k_nr1'});
              						#print "format im 5.:$k_nr1_format\n";
                          if ($k_nr1_format eq "-   " && $daten[$i-9]->{'-bab'} eq $bab)
              						{
              							$k_nr1_format = $self->format_knoten($daten[$i-9]->{'-k_nr1'});
              							#print "format im 5.:$k_nr1_format\n";
                            if ($k_nr1_format eq "-   " && $daten[$i-10]->{'-bab'} eq $bab)
              							{
              								$k_nr1_format = $self->format_knoten($daten[$i-10]->{'-k_nr1'});
              								#print "format im 5.:$k_nr1_format\n";
        ...
        # falls Knotennummer2 nur "-   " ist
        if ($k_nr2 eq "-   ")
        {
        	$k_nr2_format = $self->format_knoten($daten[$i]->{'-k_nr1'});
        	if ($k_nr2_format eq "-   ")
        	{
        		$k_nr2_format = $self->format_knoten($daten[$i-1]->{'-k_nr1'});
        		if ($k_nr2_format eq "-   ")
        		{
        			$k_nr2_format = $self->format_knoten($daten[$i-2]->{'-k_nr1'});
        			if ($k_nr2_format eq "-   ")
        			{
        				$k_nr2_format = $self->format_knoten($daten[$i-3]->{'-k_nr1'});
        				if ($k_nr2_format eq "-   ")
        				{
        					$k_nr2_format = $self->format_knoten($daten[$i-4]->{'-k_nr1'});
        					if ($k_nr2_format eq "-   ")
        					{
        						$k_nr2_format = $self->format_knoten($daten[$i-5]->{'-k_nr1'});
        						if ($k_nr2_format eq "-   ")
        						{
        							$k_nr2_format = $self->format_knoten($daten[$i-6]->{'-k_nr1'});
        							if ($k_nr2_format eq "-   ")
        							{
        								$k_nr2_format = $self->format_knoten($daten[$i-7]->{'-k_nr1'});
        								if ($k_nr2_format eq "-   ")
        								{
        									$k_nr2_format = $self->format_knoten($daten[$i-8]->{'-k_nr1'});
        									if ($k_nr2_format eq "-   ")
        									{
        										$k_nr2_format = $self->format_knoten($daten[$i-9]->{'-k_nr1'});
        										if ($k_nr2_format eq "-   ")
        										{
        											$k_nr2_format = $self->format_knoten($daten[$i-10]->{'-k_nr1'});
        										}
        										else
        										{
                							$k_nr2_format = $k_nr2_format + 1;
        										}
        									}
        									else
        									{
...
#  }
				...
				# Pfad-Hash erstellen
	# $pfad_vorher wurde als Argument mit übergeben
  # $pfad_lfd vom globalen Zähler holen und erhöhen
	$pfad_lfd = $self->{'-pfad_lfd'};
  $pfad_lfd++;
  $self->{'-pfad_lfd'} = $pfad_lfd;
  # $pfad_bab, Pfad_von und pfad_nach auch als Argumente bekommen
  # $pfad_km für diesen Pfadabschnitt wurde in dieser Funktion schon ermittelt
  # $km_ges sind nun die Gesamtkilometer, also plus vorhergehende Pfade
  $km_ges = $$pfade[$pfad_vorher]->{'-km_ges'};
  ###print "-.-km_ges:$km_ges pfad_km:$pfad_km\n";
  $km_ges = $km_ges + $pfad_km;

  $$pfade[$pfad_lfd]->{'-pfad_vorher'} = $pfad_vorher;
  $$pfade[$pfad_lfd]->{'-pfad_lfd'}    = $pfad_lfd;
  $$pfade[$pfad_lfd]->{'-pfad_bab'}    = $pfad_bab;
  $$pfade[$pfad_lfd]->{'-pfad_von'}    = $pfad_von;
  $$pfade[$pfad_lfd]->{'-pfad_nach'}   = $pfad_nach;
  $$pfade[$pfad_lfd]->{'-pfad_km'}     = $pfad_km;
  $$pfade[$pfad_lfd]->{'-km_ges'}      = $km_ges;

  print "in pfad: pfad_vorher:$pfad_vorher\n";
