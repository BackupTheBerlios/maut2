...
sub set_pfad
{
  ...
  my $pfad_nach_nr2;  # Knotennummer2 des Ziel-Teilstückes
	...
	my @daten = @$daten;       # !!! die Referenz dereferenzieren !!!
	  		...
	  		# den Scheiß mit z.B. 65a und so erstmal weggelassen
	for (my $i=0; $i<@daten; $i++)
	{
		my $bab    = $daten[$i]->{'-bab'};
	  ...
	  {
#          # den Scheiß mit z.B. 65a und so erstmal weglassen
#      if ($k_nr1 =~ /[a-zA-Z]/)
#      {
#        print "REGEXP defindet: $k_nr1\n";
#        my $pos = 0;
#        while ($pos<(length $k_nr1) and $pos != 0)
#        {
#          $pos = index($k_nr1, "a", $pos);
#          if (pos != -1)
#          {
#            substr($k_nr1, $pos, length",") = " ";
#            $pos++;
#          }
#        }
#        print "REGEXP geaendert: $k_nr1\n";
#      }
    	$pfad_von_nr1 = $k_nr1;
	  }
...
#        }
      ...
      if ($k_nr1 =~ /[a-zA-Z]/)
      {
        ...
        if ($k_nr1 >= $pfad_von_nr1 && $k_nr2 <= $pfad_nach_nr2)
        {
          $pfad_km = $pfad_km + $km;
